...
int main(int argc, char* argv[])
{
    ...
    GamePadPresent[0] = check_gamepad_present(GLFW_JOYSTICK_1);
    GamePadPresent[1] = check_gamepad_present(GLFW_JOYSTICK_2);
    GamePadPresent[2] = check_gamepad_present(GLFW_JOYSTICK_3);
    GamePadPresent[3] = check_gamepad_present(GLFW_JOYSTICK_4);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (deltaTime < 1 / 60.0f) {
            ...
            if (accumDelta > 1 / 60.0f) {
                ...
                if ((fps - 30) == 30 || (fps-30) == 0 ){
                    ...
                    Pong.prevGamepadAxis1[0] = Pong.gamepadAxis1[0];
                    Pong.prevGamepadAxis1[1] = Pong.gamepadAxis1[1];
        ...
        if (ImGui::CollapsingHeader("Time")) {
            ...
            ImGui::Text("Real Frames: %d", fps);

            float pongTime = Pong.paused ? Pong.pausedTime - Pong.elapsedTime : currentFrameTime - Pong.elapsedTime;
            ImGui::Text("Pong Time: %.3f ", pongTime); ImGui::SameLine();
            int pongFrames = Pong.paused ? Pong.pausedFrames - Pong.elapsedFrames : realFrames - Pong.elapsedFrames;
            ImGui::Text("Pong Frames: %d", pongFrames); ImGui::SameLine();
        }
        if (ImGui::CollapsingHeader("Gamepad")) {

            ImGui::SliderFloat2("[Horizontal, vertical] : ", Pong.prevGamepadAxis1, -1.0f, 1.0f);
            ImGui::Text("DiffAxis: %.3f ", Pong.diffGamepadAxis);
            ImGui::Text("DiffLeftPadX: %.3f ", Pong.diffLeftPadX);
        ...
        if (ImGui::CollapsingHeader("Misc ")) {
            ImGui::SliderFloat("rightPad length", &Pong.rightPad->Size.y, 1.0f, 400.0f);
            ImGui::SliderFloat("wall length", &Pong.wallWidth, 5.0f, 50.0f);
